...
if NOT EXIST "%data%" (
echo Youtube-dl n'est pas install‚ sur cette ordinateur.
goto fin
)
...
:aformat
"%data%\choice" /C:on /T:n,%timeout% "Modifier le format audio par defaut? (defaut:best)(%timeout%s): "
...
"%data%\youtube-dl.py" -tv --extract-audio --audio-format %format% --audio-quality %quality% "%lien%"
goto fin

:download-k
"%data%\youtube-dl.py" -tv -k --extract-audio --audio-format %format% --audio-quality %quality% "%lien%"
goto fin
...
goto test_eversion
:fin_etest
"%data%\youtube-dl.py" --version > yversion.tmp
set /p yversion=<yversion.tmp
del yversion.tmp
